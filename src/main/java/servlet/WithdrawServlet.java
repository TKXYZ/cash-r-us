...
public class WithdrawServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		// Extracts localAccount's balance (really just the withdraw amount) for maths
		double withdrawAmount = localAccount.getBalance();
		...
		if (session == null) {
			...
			try {
				...
				if ((dbBalance - withdrawAmount) < 0) {
					JOptionPane.showMessageDialog(null, "Cannot withdraw more than $" + dbBalance + "!");
					writer.write("userWithdraw.html");
				} else {
					dbBalance -= withdrawAmount;
					dbAccount.setBalance(dbBalance);
					dao.updateAccount(dbAccount);
					System.out.println("dbAccount after withdraw: " + dbAccount);
